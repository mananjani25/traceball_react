...
export const AddEmployeePage = ({ onPageChange }) => {
    ...
    const [employeeData, setEmployeeData] = useState({
        ...
        email_address: "",
        id: "",
        progress_updated_at: (1 + new Date().getMonth()).toString().padStart(2, '0') + '/' + new Date().getDate().toString().padStart(2, '0') + '/' + new Date().getFullYear(),
        current_state: 'persnal-data',
        created_at: (1 + new Date().getMonth()).toString().padStart(2, '0') + '/' + new Date().getDate().toString().padStart(2, '0') + '/' + new Date().getFullYear()
    });
    ...
    const handleFormSubmit = async (e) => {
        ...
        if (userProfile) {
            const chunks = chunkString(userProfile, 100000);    
            chunks.forEach((chunk, index) => {
                employeeData[`profile_picture_${index + 1}`] = chunk;
            });
        }
    
        employeeData.user_id = document.cookie
            .split(";")
            .find((cookie) => cookie.trim().startsWith("user_id="))
            .split("=")[1];
    
        const employee_id = Math.floor(Math.random() * (255 - 100 + 1)) + 100;
    
        if ($("#employeeForm").valid()) {
            employeeData.id = employee_id;
            console.log("Form is valid, submit the data", employeeData);
    
            const localStorageKey = `new-data-${employee_id}`;
            localStorage.setItem(localStorageKey, JSON.stringify(employeeData));
    
            document.cookie = `employee_id=${employee_id}; path=/; max-age=${60 * 60 * 24 * 7}`; // 1 week expiry
    ...
    const handleDocFormSubmit = (e) => {
        ...
        if (storedData) {
            ...
            parsedData.current_state = 'documents';
    ...
    useEffect(() => {
        ...
        $("#employeeForm").validate({
            ...
            errorPlacement: function (error, element) {
                if (element.attr("name") == "social_security_number") {
        ...
        const formatSSN = (event) => {
            ...
            setEmployeeData((prevData) => ({ ...prevData, social_security_number: formattedValue }));
        };
                ...
                <div className="right-section">
                    <div className="tabs">
                        ...
                        <button
                            ...
                            onClick={() => setActiveTab("documents")} disabled
                        >
                            Documents
                        </button>
                    </div>
                    ...
                    >
                        <form id="employeeForm" encType="multipart/form-data" onSubmit={handleFormSubmit}>
                            ...
                            <div className="form-group full-width">
                                ...
                                <input type="text" placeholder="Social Security Number" className="security_number" name="social_security_number" id="ssn" value={employeeData.social_security_number} onChange={handleInputChange} />
                            ...
                            <div className="form-group">
                                ...
                                <input type="text" placeholder="ZIP Code" className="add-feild" name="zip_code" value={employeeData.zip_code} onChange={handleInputChange} />
                                <span id="zip_error" style={{ color: 'red' }} className="error"></span>
                            </div>
                            <div className="form-group full-width">
                                <label>Date of Birth</label>
                                <input type="date" placeholder="MM/DD/YY" className="date-feild" name="date_of_birth" value={employeeData.date_of_birth} onChange={handleInputChange} />
                                <span className="error" id="dob_error"></span>
                            </div>
                            <div className="form-group half-width">
                                <label>Telephone Number</label>
                                <input type="text" placeholder="Telephone Number" className="tel-number" name="telephone_number" value={employeeData.telephone_number} onChange={handleInputChange} />
                                <span id="phone_error" style={{ color: 'red' }} className="error"></span>
                            </div>
                            <div className="form-group half-width">
                                <label>Email Address</label>
                                <input type="text" placeholder="Email Address" className="tel-number" name="email_address" value={employeeData.email_address} onChange={handleInputChange} />
                    ...
                    <div
                        className={`tab-content documentTab ${activeTab === "documents" ? "active" : "hidden"}`}
                        id="documents"
                    >
                        <form id="documentForm" encType="multipart/form-data" onSubmit={handleDocFormSubmit}>
                            ...
                            <div className="document-section">
                                <div className="Document-form-group">
                                    ...
                                    <select id="document-title-1" className="document-Selector" name="document_type_1" onChange={handleDocInputChange} value={documentData.document_type_1}>
                                        <option value="">Upload Document</option>
                                        <option value="S. PASSPORT">S. PASSPORT</option>
                            ...
                            <div className="document-section">
                                <div className="Document-form-group">
                                    ...
                                    <select id="document-title-2" className="document-Selector" name="document_type_2" onChange={handleDocInputChange} value={documentData.document_type_2}>
                                        <option value="">Upload Document</option>
                                        <option value="S. PASSPORT">S. PASSPORT</option>
                                        <option value="DRIVER LICENSE">DRIVER LICENSE</option>
                                        <option value="SOCIAL SECURITY CARD">SOCIAL SECURITY CARD</option>
